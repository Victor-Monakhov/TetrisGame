...
BaseApp::BaseApp(int xSize, int ySize) : X_SIZE(xSize), Y_SIZE(ySize)
{
	...
	COORD windowBufSize = { X_SIZE + GetSystemMetrics(SM_CXMIN), Y_SIZE + GetSystemMetrics(SM_CYMIN) };
...
void BaseApp::SetChar(int x, int y, wchar_t c, WORD color)
{
...
void BaseApp::Run()
{
	...
	while (!gameOver)
	{
		...
		{
			TCHAR  szbuff[255];
			StringCchPrintf(szbuff, 255, TEXT("FPS: %d Tetris"), counter);
